...
class ModelPaths(Serializable):
    ...
    ttt_model_path: str | None = None  # New: Path to the TTT model for context enhancement
...
class TTTArgs(Serializable):
    """Arguments for TTT (Text-to-Text Transformer) integration."""
    enable: bool = False  # Whether to use TTT for context enhancement
    context_blend: float = 0.3  # Blend factor for TTT context (0-1)
...
class TrainArgs(Serializable):
    ...
    # New: TTT context enhancement settings
    ttt: TTTArgs = field(default_factory=TTTArgs)
    
    # New: LM context length (for longer context modeling)
    ...
    def __post_init__(self) -> None:
        ...
        # New: TTT-specific validation
        if self.ttt.enable:
            if self.moshi_paths.ttt_model_path is None:
                logging.warning(
                    "TTT is enabled but no TTT model path is provided. "
                    "Make sure to set moshi_paths.ttt_model_path in your config."
                )
            if not (0.0 <= self.ttt.context_blend <= 1.0):
                raise ValueError("TTT context blend factor must be between 0.0 and 1.0")
