...
def get_fsdp_model(args: TrainArgs, checkpointer_info: CheckpointInfo) -> FullyShardedDataParallel | LMModel:
    ...
    Args:
        args (TrainArgs): A configuration object containing training arguments
            ...
            - ttt: Configuration for TTT integration (enable, context_blend).
            - context: LM context length for long-context modeling.
            - moshi_paths: Model paths including ttt_model_path.
    ...
    # Prepare model initialization kwargs with standard and TTT-specific params
    lm_kwargs_overrides = {
    ...
    # Add TTT-specific parameters if enabled
    if args.ttt.enable:
        lm_kwargs_overrides.update({
            "use_ttt_context": True,
            "ttt_model_path": args.moshi_paths.ttt_model_path,
            "ttt_context_blend": args.ttt.context_blend,
        })
        main_logger_info(f"TTT context enhancement enabled with blend factor {args.ttt.context_blend}")
        
    # Add context length if specified
    if args.context is not None:
        lm_kwargs_overrides["context"] = args.context
        main_logger_info(f"Using LM context length of {args.context}")
    ...
    with torch.device("meta"):
        model = checkpointer_info.get_moshi(
            ...
            # Removed the 'load_weight=False' parameter that was causing the error
        )
