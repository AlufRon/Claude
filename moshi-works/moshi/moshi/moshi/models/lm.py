...
from ..models.ttt_processor import TTTContextProcessor
...
class LMModel(StreamingContainer):
    ...
    def __init__(
        ...
        use_ttt_context: bool = False,  # Add explicit TTT parameters
        ttt_model_path: tp.Optional[str] = None,
        ttt_context_blend: float = 0.3,
        **kwargs,
    ):
        super().__init__()
        self.n_q = n_q
        ...
        # num_codebooks property returns n_q + 1
        ...
        # Create a copy of kwargs without TTT params to avoid passing them downstream
        filtered_kwargs = {k: v for k, v in kwargs.items() 
                         if k not in ["use_ttt_context", "ttt_model_path", "ttt_context_blend"]}
        
        main_kwargs = {
            k: v for k, v in filtered_kwargs.items() if not k.startswith(depformer_prefix)
        }
        ...
        self.use_ttt_context = use_ttt_context
        self.ttt_context_blend = ttt_context_blend
        self.ttt_processor = None
        if use_ttt_context and ttt_model_path:
            # FIX: Use 4096 here instead of "dim" so that TTT produces a context
            # embedding with the same dimension as the transformer output.
            self.ttt_processor = TTTContextProcessor(
                ttt_model_path, 
                4096,  # Hidden size 
                device=device,
                keep_model_loaded=True,  # Keep model in memory between calls
                verbose=False,  # Reduce logging noise in production
                expected_codebooks=8  # Match your expected codebook count
            )
    @property
    ...
    def forward(
        ...
        # Add TTT context enhancement
        if self.use_ttt_context and self.ttt_processor is not None:
            # --- START MODIFICATION ---
            # Extract both Moshi's and user's audio tokens for TTT processing
            B = codes.shape[0]
            T_codes = codes.shape[-1]  # Get time dimension from input 'codes'
            K_depformer = self.dep_q  # Number of codebooks TTT should process per stream (e.g., 8)
            K_moshi_audio = self.n_q  # Total Moshi audio codebooks (e.g., 8)

            # 1. Extract Moshi's first K_depformer audio tokens
            moshi_token_start_idx = self.audio_offset  # Usually 1 (skip text)
            moshi_token_end_idx = moshi_token_start_idx + K_depformer
            moshi_tokens = codes[:, moshi_token_start_idx:moshi_token_end_idx]
            # Shape: [B, K_depformer, T_codes]

            # 2. Extract User's first K_depformer audio tokens
            # Assuming user acoustic tokens start right after Moshi's streams
            # Example layout [Text, MoshiAudio1..Q, UserAudio1..Q]
            user_stream_start_idx = 1 + K_moshi_audio
            user_token_end_idx = user_stream_start_idx + K_depformer
            
            # Ensure indices are valid for the 'codes' tensor shape
            if codes.shape[1] >= user_token_end_idx:
                user_tokens = codes[:, user_stream_start_idx:user_token_end_idx]
                # Shape: [B, K_depformer, T_codes]
            else:
                # Handle cases where user stream might not be present or fully available
                # For now, create zero tokens as placeholders if missing
                print(f"Warning: User audio stream missing or incomplete in 'codes' tensor (shape: {codes.shape}). Using zeros for TTT input.")
                user_tokens = torch.zeros_like(moshi_tokens)  # Create placeholder tokens

            # 3. Combine the token streams (Strategy: Stack Codebooks)
            # Resulting shape: [B, K_depformer * 2, T_codes]
            if moshi_tokens.shape[-1] != user_tokens.shape[-1]:
                # Handle potential time dimension mismatch
                print(f"Warning: Mismatch in time dimension for TTT input: Moshi {moshi_tokens.shape}, User {user_tokens.shape}")
                # Fallback: duplicate moshi tokens if user tokens invalid
                combined_tokens = torch.stack([moshi_tokens, moshi_tokens], dim=2).view(
                    B, K_depformer * 2, T_codes
                )
            else:
                combined_tokens = torch.stack([moshi_tokens, user_tokens], dim=2).view(
                    B, K_depformer * 2, T_codes
                )

            # 4. Pass the combined sequence to the processor
            # Ensure combined_tokens is on the correct device before passing
            combined_tokens = combined_tokens.to(self.ttt_processor.device)
            ttt_context = self.ttt_processor.get_context_embedding(combined_tokens)
            # --- END MODIFICATION ---
...
class LMGen(StreamingModule[_LMGenState]):
    ...
    def step(self, input_tokens: torch.Tensor) -> torch.Tensor | None:
        ...
        # Call the main transformer forward pass (graphed)
        transformer_out, text_logits = state.graphed_main(input_, state.condition_sum)

        # --- START: TTT Context Calculation and Blending Logic ---
        if self.lm_model.use_ttt_context and self.lm_model.ttt_processor is not None:
            # --- Prepare audio tokens for TTT ---
            device = state.cache.device
            offset = self.lm_model.audio_offset
            dep_q = self.lm_model.dep_q
            position = state.offset % state.cache.shape[2]

            # Create/update persistent buffer for recent tokens
            if not hasattr(state, 'token_buffer'):
                # Initialize buffer, using class attribute instead of local variable
                print(f"---ttt-initializing-token-buffer-size-{self.ttt_buffer_size}---")
                state.token_buffer = torch.zeros(state.batch_size, dep_q, self.ttt_buffer_size, dtype=torch.long, device=device)
                
                # Also create a user token buffer with same size
                state.user_token_buffer = torch.zeros(state.batch_size, dep_q, self.ttt_buffer_size, dtype=torch.long, device=device)

            # Get the most recent generated tokens for the relevant codebooks
            if state.offset > 0:  # Ensure we are not at the very first step
                # Get Moshi tokens from cache
                moshi_tokens = state.cache[:, offset:offset+dep_q, position:position+1]
                # Shift buffer and add new tokens
                state.token_buffer = torch.cat([state.token_buffer[:, :, 1:], moshi_tokens], dim=2)
                
                # Get User tokens if available (from input_tokens)
                if input_tokens.shape[1] >= dep_q:
                    user_tokens = input_tokens[:, :dep_q]
                    # Shift buffer and add new user tokens
                    state.user_token_buffer = torch.cat([state.user_token_buffer[:, :, 1:], user_tokens], dim=2)

            # Combine Moshi and User token buffers - using self.ttt_buffer_size instead of buffer_size
            combined_tokens = torch.stack([state.token_buffer, state.user_token_buffer], dim=2).view(
                state.batch_size, dep_q * 2, self.ttt_buffer_size
            )
            
            print("---ttt-streaming-tokens-shape---", combined_tokens.shape)  # Log input shape

            # --- Call TTT Processor ---
            print("---ttt-getting-context-embedding---")  # Log before call
            ttt_context = self.lm_model.ttt_processor.get_context_embedding(combined_tokens)
            print("---ttt-context-embedding-raw-shape---", ttt_context.shape)  # Log output shape

            # --- Adapt TTT Context Shape for Blending ---
            # Expected transformer_out shape: [B_cfg, 1, Dim]
            # Adapt ttt_context [B, T_ctx, Dim] -> [B_cfg, 1, Dim]
            if ttt_context.shape[1] > 1:
                # Use only the context from the latest time step corresponding to the buffer
                ttt_context = ttt_context[:, -1:, :]
                print("---ttt-context-embedding-adapted-shape---", ttt_context.shape)  # Log adapted shape

            # Repeat for CFG if necessary
            if self.cfg_coef != 1. and ttt_context.shape[0] != transformer_out.shape[0]:
                ttt_context_repeated = ttt_context.repeat(2, 1, 1)
                print("---ttt-context-embedding-repeated-for-cfg---", ttt_context_repeated.shape)  # Log repeated shape
            else:
        ...
        # Pass the (potentially modified) transformer_out to the depformer step
