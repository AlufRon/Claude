...
class CheckpointInfo:
    ...
    def get_moshi(
        ...
        """Returns a pretrained Moshi model."""
        # Call the global function with the class attributes
        return get_moshi_lm(
            self.moshi_weights, 
            self.lm_config,
            device=device,
...
def get_moshi_lm(
    ...
    if filename is not None:
        # <<< MODIFICATION START >>> Load state dict to CPU first, then into model
        if _is_safetensors(filename):
            # Load state dict to CPU
            print(f"[get_moshi_lm] Loading main LM state dict '{filename}' to CPU...")
            state_cpu = load_file(filename, device="cpu") # Load to CPU
            print(f"[get_moshi_lm] Converting state dict to {dtype}...")
            # Convert dtypes on CPU
            for key, value in state_cpu.items():
                if isinstance(value, torch.Tensor) and value.is_floating_point():
                     state_cpu[key] = value.to(dtype=dtype)

            # Load into the potentially meta model structure using assign=True
            print(f"[get_moshi_lm] Loading state dict into model structure (assign=True)...")
            # Use strict=False initially as checkpoint might only contain base model weights, not LoRA etc.
            missing_keys, unexpected_keys = model.load_state_dict(state_cpu, strict=False, assign=True)
            if missing_keys:
                print(f"[get_moshi_lm] WARNING: Missing keys: {missing_keys}")
            if unexpected_keys:
                print(f"[get_moshi_lm] WARNING: Unexpected keys: {unexpected_keys}")
            print(f"[get_moshi_lm] State dict loaded.")
            del state_cpu # Free CPU memory

        else: # Handling non-safetensors format (e.g., .pt) - also modified to use assign=True
            print(f"[get_moshi_lm] Loading non-safetensors checkpoint '{filename}' to CPU...")
            pkg = torch.load(filename, "cpu") # Load to CPU
            # Try to find the state dict under common keys
            state_dict = pkg.get("fsdp_best_state", {}).get("model", pkg.get("model", pkg))
            if not state_dict:
                 raise ValueError(f"Could not find model state_dict in {filename}")
            ...
            if unexpected_keys:
                print(f"[get_moshi_lm] WARNING: Unexpected keys: {unexpected_keys}")
            print(f"[get_moshi_lm] State dict loaded.")
            del state_dict, pkg # Free memory
