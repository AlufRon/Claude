...
class TTTCache:
    ...
    def update(self, py_tree, layer_idx, seq_len):
        old_weights = self.ttt_params_dict["W1_states"][layer_idx].clone().mean().item()
        ...
        new_weights = self.ttt_params_dict["W1_states"][layer_idx].mean().item()
        weight_diff = new_weights - old_weights
        print(f"Layer {layer_idx} weights changed by {weight_diff:.6f} (old: {old_weights:.6f}, new: {new_weights:.6f})")
